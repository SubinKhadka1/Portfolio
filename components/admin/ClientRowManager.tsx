...
function AddToRowButton({ row, variant = "header" }: { row: number; variant?: "header" | "empty" }) {
  const href = addClientHref(row);
...
function ClientRowCard({
  ...
  return (
    <article
      ...
      onDrop={onDrop}
      className="client-row-card shrink-0 snap-start bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden hover:border-purple-500/40 transition-colors flex flex-col"
    >
      <div className="relative p-1.5 pb-0">
        <div
          className={`client-row-logo mx-auto rounded-md bg-gray-100 border border-zinc-700/40 flex items-center justify-center p-2 ${containerClass}`}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={project.media_url}
            alt={project.title}
            className={`client-row-logo-img object-contain ${className}`}
          />
        </div>
...
export default function ClientRowManager({
  projects: initial,
  clientRows,
}: {
  projects: Project[];
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl px-3 sm:px-4 py-3 text-xs sm:text-sm text-zinc-400 leading-relaxed">
        ...
        matching your homepage clients section. Use <strong className="text-zinc-300">Add to row</strong>{" "}
        on any row, drag to reorder, or move logos between rows. Change row count in{" "}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-3 sm:px-4 py-3 border-b border-zinc-800 bg-zinc-900/80">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-zinc-500 text-xs mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
                  ...
                  <span>
                    {rowProjects.length} client{rowProjects.length === 1 ? "" : "s"}
                  </span>
            ...
            {rowProjects.length === 0 ? (
              ...
              >
                <p className="text-zinc-500 text-sm mb-4">
                  No clients in row {rowNum} yet. Add one or drag a logo here from another row.
                </p>
